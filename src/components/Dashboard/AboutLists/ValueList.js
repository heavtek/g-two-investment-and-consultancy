...
setValueList(tableMeta.rowData[2]);
      ...
      onClick={async ()=>{
          await deleteData('values-list',tableMeta.rowData[0])
          await getAboutdata()
      }} >delete</Button>
...
const getAboutdata=async()=>{
  const data=await getAllData('values-list');
...
setValueList('')
    ...
    >
      ...
      <TextField id="outlined-basic" value={valueList} onChange={(e)=>{
setValueList(e.target.value)
      }
      } label="Our Value"   variant="outlined" color="success" />


      <Stack direction="row" spacing={2}>
      <Button variant="contained"   onClick={async()=>{
         if(editdata){
          await updateData('values-list',{valueList,valueId},aboutid);
          await getAboutdata()
          setEditdata(false);
          setManAbout(false);
         }else{
          await postData('values-list',{valueList,valueId});
          await getAboutdata();
           setEditdata(false);
           setManAbout(false);
         }
