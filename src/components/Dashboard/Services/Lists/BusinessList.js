...
const BusinessList =(props) =>{
  const [businessListName,setBusinessListName]=useState("");
     ...
     return <>
      ...
      <Button type="button"  variant="contained" color="success"
       onClick={async ()=>{
          ...
          setBusinessListName(tableMeta.rowData[2]);
      ...
      onClick={async ()=>{
          await deleteData('business-list',tableMeta.rowData[0])
          getAboutdata()
      }} >delete</Button>
        </>
  }
...
const getAboutdata=async()=>{
  const data=await getAllData('business-list');
  ...
  setBusinessId(props.businessId)       
   ...
   setBusinessListName('')
    ...
    >
      ...
      <TextField id="outlined-basic" value={businessListName} onChange={(e)=>{
        setBusinessListName(e.target.value)
      }
...
<TextField id="outlined-basic" multiline={true}  value={listDescription} onChange={(e)=>{
      ...
      <Button variant="contained"   onClick={async()=>{
         if(editdata){
          await updateData('business-list',{businessListName,listDescription,businessId},aboutid);
          await getAboutdata()
          setEditdata(false);
          setManAbout(false);
         }else{
          await postData('business-list',{businessListName,listDescription,businessId});
          await getAboutdata();
           setEditdata(false);
           setManAbout(false);
         }
