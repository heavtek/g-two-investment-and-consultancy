     ...
     return <>
      ...
      <Button type="button"  variant="contained" color="success"
       onClick={async ()=>{
          ...
          setTrainingListName(tableMeta.rowData[2]);
      ...
      onClick={async ()=>{
          await deleteData('training-list',tableMeta.rowData[0])
          getAboutdata()
      }} >delete</Button>
        </>
  }
...
const getAboutdata=async()=>{
  const data=await getAllData('training-list');
  ...
  setTrainingId(props.trainingId)       
   ...
   setTrainingListName('')
    ...
    >
      ...
      <TextField id="outlined-basic" value={trainingListName} onChange={(e)=>{
        setTrainingListName(e.target.value)
      }
...
<TextField id="outlined-basic" multiline={true}  value={listDescription} onChange={(e)=>{
      ...
      <Button variant="contained"   onClick={async()=>{
         if(editdata){
          await updateData('training-list',{trainingListName,listDescription,trainingId},aboutid);
          await getAboutdata()
          setEditdata(false);
          setManAbout(false);
         }else{
          await postData('training-list',{trainingListName,listDescription,trainingId});
          await getAboutdata();
           setEditdata(false);
           setManAbout(false);
         }
